...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    early_stopping_patience: int = field(default=3, metadata={"help": 'How many steps to wait before stopping training if no improvement is seen'})
...
def local_dataset(dataset_dir):
    train_dataframes = []
    for i, split_file in enumerate(sorted(os.listdir(dataset_dir))):
      if split_file.endswith('.csv'):
          split_ds = pd.read_csv(os.path.join(dataset_dir, split_file))
      elif split_file.endswith('.tsv'):
          split_ds = pd.read_csv(os.path.join(dataset_dir, split_file), delimiter='\t')
      else:
          raise ValueError(f"Unsupported dataset format: {split_ds}")

      train_dataframes.append(split_ds)
    
    split_dataset =  Dataset.from_pandas(pd.concat(train_dataframes), preserve_index=False).train_test_split(test_size=0.1)
